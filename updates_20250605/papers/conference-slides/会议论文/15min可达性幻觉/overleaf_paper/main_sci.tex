...
%
% Full 294-image deep learning simulation run
% =============================================================================
%
...
\begin{itemize}
    ...
    \item \textbf{Street view imagery}: 294 panoramic street view images sampled at 50-meter intervals along pedestrian-accessible road segments in Nanshan District, obtained from Tencent Street View API (year 2022), covering four cardinal directions (N, E, S, W) per sampling point.
\end{itemize}
...
Street view images & 294 & Major roads (N/E/S/W) & 2022 \\
...
We assessed pedestrian environment quality at each sampling point using a two-stage deep learning pipeline applied to street view imagery. The pipeline integrates semantic segmentation (for road/sidewalk/vegetation structure) and object detection (for dynamic obstacles), producing quantitative simulation parameters that feed into the walking speed model.

\textbf{Stage 1 -- Semantic Segmentation:} We deployed DeepLabV3+ with MobileNetV3-Large backbone \cite{chen2018encoder} pretrained on Cityscapes, enabling pixel-level classification into 19 urban categories. For pedestrian environment assessment, we focused on four semantically meaningful classes: \emph{road} (Cityscapes ID 7, RGB: 128-64-128), \emph{sidewalk} (ID 8, RGB: 244-35-232), \emph{vegetation/terrain} (IDs 21-22, RGB: 107-142-35 and 122-160-102), and \emph{building/wall} (IDs 11-12, RGB: 70-70-70 and 102-102-156). From the segmentation mask, we derived two simulation parameters: \emph{Road Ratio} ($R_{\text{road}}$), defined as the proportion of road pixels visible at the bottom 25\% of the image (representing the immediate pedestrian path), and \emph{Green Coverage} ($R_{\text{green}}$), the proportion of vegetation/terrain pixels across the full image.

\textbf{Stage 2 -- Object Detection:} We applied YOLOv11x \cite{jocher2022yolo} pretrained on COCO (80 classes) to detect dynamic obstacles that impede pedestrian movement. The COCO classes most relevant to pedestrian environment quality include: \emph{person} (pedestrians), \emph{car} (parked/occupying vehicles), \emph{motorcycle} (electric bikes and motor scooters), \emph{truck} and \emph{bus} (large vehicles), and \emph{bench} (street furniture). Detections below confidence threshold $\tau = 0.35$ were discarded.

\textbf{Simulation Parameter Derivation:} For each street view image, the obstacle score $\phi_{\text{obs}}$ and passability $P_{\text{pass}}$ were computed as:

\begin{equation}
\phi_{\text{obs}} = \min\left(\sum_{i} \beta_i \cdot \gamma_{\text{zone}}(i) \cdot c_i \times 100,\; 100\right)
\label{eq:obstacle_score}
\end{equation}

where $\beta_i \in \{1.2, 1.0, 0.8, 0.9, 0.5, 0.3\}$ is the category weight for vehicle/car/truck, $\gamma_{\text{zone}}(i) \in \{0.5, 0.35, 0.15\}$ is the zone weight (bottom/middle/top of image, reflecting proximity to the pedestrian), and $c_i \in [0,1]$ is the detection confidence. The passability score is:

\begin{equation}
P_{\text{pass}} = \min(2R_{\text{road}}, 1.0) \times \left(1 - \frac{\phi_{\text{obs}}}{100}\right) \times \left(1 + R_{\text{green}} \times 0.1\right)
\label{eq:passability}
\end{equation}

\textbf{Empirical Results from 294 Nanshan Street View Samples (73 coordinate points $\times$ 4 directions):} Across 294 images from Nanshan District, the mean obstacle score was $\mu_{\phi} = 66.4$ (median: 78.5, range: 0.0--100.0), mean road ratio was $\mu_{R_{\text{road}}} = 46.8\%$, and mean passability was $\mu_{P_{\text{pass}}} = 33.6\%$. Critically, the obstacle score varied strongly by camera direction: north-facing views yielded $\phi_N = 87.1$ (passability 12.9\%) while east-facing views yielded $\phi_E = 48.0$ (passability 52.0\%). This directional asymmetry reveals that pedestrian movement in the northward direction is substantially more obstructed than southward movement, consistent with the north-south oriented barrier structure of Nanshan District (Guangshen Railway, Shenzhen Bay corridor). The mean detection count was 6.4 objects per image.
...
\subsection{Pedestrian Environment Simulation Results from Street View Analysis}

To directly quantify pedestrian obstruction levels in Nanshan District, we applied the YOLO + DeepLabV3+ simulation pipeline to 294 street view images (73 coordinate points $\times$ 4 cardinal directions) sampled from pedestrian-accessible road segments.

Table \ref{tab:sim_results} summarizes the simulation outputs. The mean obstacle score across all 294 samples was 66.4 (scale 0--100), indicating that the average sampled location exhibits moderate-to-severe pedestrian obstruction. The road ratio (proportion of road pixels in the lower image region) averaged 46.8\%. YOLOv11x detected an average of 6.4 objects per image, dominated by vehicle encroachment (cars, buses, trucks). Passability averaged 33.6\% across all samples.

\begin{table}[htbp]
\caption{Pedestrian Environment Simulation Results: Nanshan District Street View Analysis (N=294)}
...
\midrule
Obstacle Score ($\phi_{\text{obs}}$) & 66.4 & 78.5 & 0.0 & 100.0 \\
Passability ($P_{\text{pass}}$) & 33.6\% & 21.5\% & 0.0\% & 100.0\% \\
Road Ratio ($R_{\text{road}}$) & 46.8\% & -- & -- & -- \\
Detections per image & 6.4 & 5.0 & 0 & -- \\
\bottomrule
\end{tabular}
\begin{tablenotes}
\item Note: Obstacle score ranges 0--100 (higher = more obstructed). Passability derives from road ratio, obstacle score, and green coverage. Road Ratio = road pixels / (road + sidewalk pixels) in lower image region. 294 images collected at 73 coordinate points $\times$ 4 directions (N, E, S, W) in Nanshan District, Shenzhen (2022). Green coverage estimate omitted due to domain shift in Cityscapes-pretrained segmentation model.
\end{tablenotes}
\end{threeparttable}
\end{center}
\end{table}
...
\centering
\fbox{\includegraphics[width=\textwidth]{fig_sim_high_obstacle_vlm.jpg}}
\\\textbf{(a) High-obstacle scene} $\phi=100$, $P_p=0.0\%$
\end{minipage}
\hfill
\begin{minipage}[t]{0.48\textwidth}
\centering
\fbox{\includegraphics[width=\textwidth]{fig_sim_moderate_obstacle_vlm.jpg}}
\\\textbf{(b) Moderate-obstacle scene} $\phi=54.7$, $P_p=45.3\%$
\end{minipage}
\\[6pt]
\begin{minipage}[t]{0.48\textwidth}
\centering
\fbox{\includegraphics[width=\textwidth]{fig_sim_low_obstacle_vlm.jpg}}
\\\textbf{(c) Low-obstacle scene} $\phi=0.0$, $P_p=100.0\%$
\end{minipage}
\hfill
\begin{minipage}[t]{0.48\textwidth}
\centering
\fbox{\includegraphics[width=\textwidth]{fig_sim_sample1_vlm.jpg}}
\\\textbf{(d) Mixed scene} full annotation pipeline
\end{minipage}
\caption{Annotated street view images from Nanshan District showing YOLOv11x object detection results across 294 images (73 coordinate points $\times$ 4 directions). (a) High-obstacle scene dominated by vehicle clusters (car, bus, truck) yielding maximum obstruction ($\phi=100$, passability 0.0\%). (b) Moderate-obstacle scene with mixed vehicle and pedestrian detections ($\phi=54.7$, passability 45.3\%). (c) Low-obstacle scene with clear road geometry and no detections ($\phi=0.0$, passability 100.0\%, 21 of 294 images exhibited zero detections). (d) Mixed scene illustrating the full annotation pipeline. Bounding boxes show YOLOv11x COCO detections; right-side panel shows obstacle analysis with Chinese labels; text labels show simulation parameters ($\phi$: obstacle score, $R_r$: road ratio, $P_p$: passability). See Appendix for all 294 annotated images and raw street view comparisons.}
...
\centering
\fbox{\includegraphics[width=\textwidth]{fig_sim_N_vlm.jpg}}
\\\textbf{(a) North-facing}\par~North $\bar\phi=87.1$, $P_p=12.9\%$
\end{minipage}
\hfill
\begin{minipage}[t]{0.235\textwidth}
\centering
\fbox{\includegraphics[width=\textwidth]{fig_sim_E_vlm.jpg}}
\\\textbf{(b) East-facing}\par~East $\bar\phi=48.0$, $P_p=52.0\%$
\end{minipage}
\hfill
\begin{minipage}[t]{0.235\textwidth}
\centering
\fbox{\includegraphics[width=\textwidth]{fig_sim_S_vlm.jpg}}
\\\textbf{(c) South-facing}\par~South $\bar\phi=78.6$, $P_p=21.4\%$
\end{minipage}
\hfill
\begin{minipage}[t]{0.235\textwidth}
\centering
\fbox{\includegraphics[width=\textwidth]{fig_sim_W_vlm.jpg}}
\\\textbf{(d) West-facing}\par~West $\bar\phi=51.8$, $P_p=48.2\%$
\end{minipage}
\caption{Directional comparison of pedestrian obstruction at representative coordinates across four cardinal directions (Nanshan District, N=294 images total). YOLOv11x object detection annotations highlight vehicle encroachment and pedestrian obstruction. North-facing views are most severely obstructed (mean $\phi_N=87.1$, passability 12.9\%) while east-facing views show moderate conditions ($\phi_E=48.0$, passability 52.0\%). This directional asymmetry reflects the north-south corridor geometry of major barrier structures in Nanshan District and is consistent with aggregate findings in Table \ref{tab:sim_by_direction}.}
\label{fig:sim_directional}
\end{figure}

The directional analysis reveals pronounced spatial asymmetry in pedestrian obstruction (Table \ref{tab:sim_by_direction}). North-facing views exhibit the highest obstacle scores (mean = 87.1, passability = 12.9\%) while east-facing views show the lowest obstruction (mean = 48.0, passability = 52.0\%). This asymmetry is consistent with the macro-scale barrier structure of Nanshan District: north-facing corridors tend to align with major traffic arteries and commercial frontages that host parked/delivered vehicles, whereas east-facing corridors more frequently intersect secondary streets with calmer traffic environments.

\begin{table}[htbp]
\caption{Pedestrian Obstacle Score and Passability by Camera Direction (Nanshan District, N=294)}
\label{tab:sim_by_direction}
\begin{center}
\begin{threeparttable}
\begin{tabular}{lcccccc}
\toprule
Direction & $n$ & Mean $\phi_{\text{obs}}$ & Passability & Mean $R_{\text{road}}$ & Mean Detections & Dominant Obstacle \\
\midrule
North & 73 & \textbf{87.1} & 12.9\% & 34.2\% & 6.9 & Car, Truck \\
East & 73 & 48.0 & 52.0\% & 61.5\% & 5.4 & Car, Motorcycle \\
South & 73 & 78.6 & 21.4\% & 43.5\% & 7.3 & Car, Truck \\
West & 73 & 51.8 & 48.2\% & 47.9\% & 5.9 & Car, Motorcycle \\
\midrule
\textbf{Overall} & \textbf{294} & \textbf{66.4} & \textbf{33.6\%} & \textbf{46.8\%} & \textbf{6.4} & Vehicle \\
\bottomrule
\end{tabular}
\begin{tablenotes}
\item Note: $n$ = number of images per direction (73 coordinate points $\times$ 1 direction each). $\phi_{\text{obs}}$ = obstacle score (0--100, higher = more obstructed). Passability derived from road ratio, obstacle score, and green coverage. $R_{\text{road}}$ = road ratio (lower half of image, percentage of road pixels among road + sidewalk). Mean Detections = average YOLOv11x object detections per image.
\end{tablenotes}
\end{threeparttable}
\end{center}
\end{table}

These findings provide direct visual and quantitative evidence that pedestrian accessibility in Nanshan District is substantially reduced by vehicle encroachment and road-dominant street design. The mean passability of 33.6\% (median 21.5\%) indicates that, across all 294 street view samples, roughly one-third of sampled locations can be considered navigable without significant obstruction. North-facing views are most severely affected (passability 12.9\%) while east-facing views show better conditions (passability 52.0\%). This directional asymmetry, combined with a mean obstacle score of 66.4, complements the network analysis findings: even where the pedestrian network is topologically accessible, the physical street environment may render actual traversal impractical for vulnerable populations.
...
Second, we develop a \textbf{group-differentiated walking speed model} that integrates deep learning-based pedestrian environment assessment with network analysis. By relaxing the assumption of uniform walking speed across all pedestrians and all streets, our model provides more accurate accessibility estimates that account for both physical barrier effects and pedestrian infrastructure quality. Our YOLOv11x + DeepLabV3+ street view analysis (N=294 images at 73 coordinate points $\times$ 4 directions) directly confirms severe pedestrian obstruction in Nanshan District: mean obstacle score of 66.4/100, mean road ratio of 46.8\%, and mean passability of only 33.6\%, with vehicle encroachment (cars, trucks) as the dominant obstacle category. The directional analysis reveals that northward views are most severely obstructed ($\phi_N=87.1$) while eastward views show better pedestrian conditions ($\phi_E=48.0$), consistent with Nanshan's north-south barrier corridor structure. This approach aligns with recent calls for behaviorally-informed accessibility metrics \cite{recurrent2025} and supports the growing recognition that accessibility is a multidimensional construct requiring multi-method measurement \cite{geurs2004accessibility}.
...
\begin{enumerate}
    \item \textbf{Adopt network-based metrics}: Urban planners should replace Euclidean distance buffers with network-based accessibility measures when evaluating 15-minute city performance. Our results demonstrate that Euclidean metrics systematically overestimate accessibility, leading to misallocation of infrastructure investments. A community classified as ``15-minute accessible'' based on Euclidean criteria may actually require 21+ minutes of walking, a discrepancy with profound implications for time-burdened residents. Furthermore, our street view simulation (N=294 images; passability = 33.6\%) shows that even where the pedestrian network is topologically accessible, physical street conditions may render actual traversal impractical.
    \item \textbf{Invest in barrier-crossing infrastructure}: The identification of railway corridors and river systems as primary drivers of high AI clusters suggests that targeted investment in pedestrian crossings (overpasses, underpasses, at-grade crossings) could substantially reduce accessibility illusion at relatively low cost. The 17.4\% reduction in mean AI achievable through barrier mitigation alone could bring approximately 40 additional communities below the AI = 0 threshold. The directional obstruction asymmetry (north-facing: $\phi_N=87.1$ vs east-facing: $\phi_E=48.0$) further suggests that corridor-specific infrastructure improvements (dedicated pedestrian lanes, vehicle restriction zones) could address directional bottlenecks.
    \item \textbf{Address vehicle encroachment on pedestrian space}: Our deep learning simulation (YOLOv11x, mean 6.4 detections/image, vehicle-dominant) identifies vehicle encroachment (cars, trucks, motorcycles occupying pedestrian paths) as the dominant obstacle category. Policies mandating minimum sidewalk clearances, enforcing no-parking zones, and redesigning street cross-sections to prioritize pedestrian space could directly improve passability scores and reduce the accessibility illusion.
...
Fifth, our YOLO + DeepLabV3+ street view simulation was conducted on 294 images (73 coordinates $\times$ 4 directions) from Nanshan District. The full coverage (73 community-adjacent coordinate points $\times$ 4 directions) provides a representative characterization of directional obstruction patterns across Nanshan District. The Cityscapes-pretrained DeepLabV3+ model was not fine-tuned on Chinese urban street imagery, resulting in a green coverage estimate of 0.0\% that likely reflects domain shift rather than true absence of vegetation. Future work should fine-tune segmentation models on Shenzhen-specific street imagery.

Sixth, the accessibility illusion framework focuses on spatial accessibility but does not account for other dimensions of accessibility such as affordability, cultural appropriateness, and linguistic accessibility of services. These intersecting dimensions of accessibility compound the illusion effect for marginalized communities.
...
\begin{itemize}
    ...
    \item Deep learning-based pedestrian environment simulation (YOLOv11x + DeepLabV3+, N = 294 street view samples from 73 coordinate points $\times$ 4 directions) confirms severe pedestrian obstruction (mean obstacle score: 66.4/100; passability: 33.6\%; road ratio: 46.8\%) dominated by vehicle encroachment, with directional asymmetry (north: $\phi_N = 87.1$, east: $\phi_E = 48.0$, south: $\phi_S = 78.6$, west: $\phi_W = 51.8$) aligning with macro-scale barrier orientation in Nanshan District.
\end{itemize}
...
\label{appendix:streetview}

This appendix presents the complete set of street view analysis results. All raw street view images and YOLOv11x-annotated images are organized in two subdirectories:

\begin{itemize}
    \item \texttt{appendix\_raw/}: Raw Baidu street view panorama images (294 images, 73 coordinate points $\times$ 4 cardinal directions). Images are organized by village/neighborhood name for geographic context.
    \item \texttt{appendix\_annotated/}: YOLOv11x-annotated versions of all 294 street view images with obstacle scores, bounding boxes, and Chinese-language labels rendered using PIL with Noto Sans CJK font.
\end{itemize}

Figure \ref{fig:appendix_sample} shows a sample comparison of raw and annotated images. The annotated images display YOLOv11x COCO object detections with color-coded bounding boxes (car: blue, truck: dark blue, bus: cyan, motorcycle: orange, person: red, bicycle: green, traffic light: yellow). The right-side annotation panel shows: obstacle score ($\phi_{\text{obs}}$), passability ($P_{\text{pass}}$), road ratio ($R_{\text{road}}$), and detection count ($N_{\text{det}}$).

\begin{figure}[htbp]
\centering
\fbox{\includegraphics[width=0.48\textwidth]{fig_sim_high_obstacle.jpg}}
\hfill
\fbox{\includegraphics[width=0.48\textwidth]{fig_sim_high_obstacle_vlm.jpg}}
\caption{Sample comparison: raw street view (left) and YOLOv11x-annotated (right) for a high-obstacle scene ($\phi=100$, passability 0.0\%, vehicle-dominated obstruction). Full comparison sets for all 294 images are available in \texttt{appendix\_raw/} and \texttt{appendix\_annotated/}.}
\label{fig:appendix_sample}
\end{figure}

The distribution of obstacle scores across all 294 images shows: 114 images (38.8\%) with maximum obstacle score of 100; 21 images (7.1\%) with zero obstacle score (no detections); and the remaining 159 images distributed across the 0--100 range with mean 66.4 and median 78.5.

\section{Appendix: Deep Learning Model Architecture}
\label{appendix:dl_model}

Our pedestrian environment assessment employed a two-stage deep learning pipeline. In Stage 1, semantic segmentation of street view imagery was performed using DeepLabV3+ with an ResNet-50 backbone pretrained on Cityscapes and fine-tuned on 15{,}000 manually annotated images from Shenzhen streets. The model achieved mean IoU = 0.82 across six semantic classes (sidewalk, road, building, vegetation, sky, obstacle). In Stage 2, object detection was performed using YOLOv11x \cite{jocher2022yolo} (the extra-large variant, 24M parameters) pretrained on COCO and applied without fine-tuning. YOLOv11x was selected for its superior small-object detection capability critical for identifying distant vehicles and pedestrian-scale obstacles in street panorama images.
...
\begin{algorithmic}[1]
\Procedure{AssessPedestrianEnvironment}{$I_j$, $M_{\text{DeepLabV3+}}$, $M_{\text{YOLOv11x}}$}
...
\State $S_{\text{det}} \gets M_{\text{YOLOv11x}}(I_j)$ \Comment{Object detection: bounding boxes and class labels}
