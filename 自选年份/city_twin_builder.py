...
示例:
    ...
    parser.add_argument("--step", default=None, help="单独运行指定步骤: illusion, scoring, verification")
    parser.add_argument("--skip-scoring", action="store_true", help="跳过幻觉评分步骤")
    parser.add_argument("--skip-verification", action="store_true", help="跳过路径验证步骤")
    ...
    # 单独运行幻觉验证步骤
    if args.step in ("illusion", "scoring", "verification"):
        log.info(f"# 可达性幻觉验证模块 (step={args.step})")
        log.info(f"# {'#' * 50}")
        ...
        if args.step in ("illusion", "scoring") and not args.skip_scoring:
            spec = importlib.util.spec_from_file_location(
                "illusion_scorer",
                SCRIPT_DIR / "illusion_scorer.py"
            )
            ...
            stats_candidates = [
                SCRIPT_DIR / "network_output" / "walkable_stats.json",
            ]

            sv_path = next((p for p in sv_candidates if p.exists()), None)
            ns_path = next((p for p in ns_candidates if p.exists()), None)
            stats_path = next((p for p in stats_candidates if p.exists()), None)

            if sv_path and stats_path:
                out = SCRIPT_DIR / "illusion_output"
                out.mkdir(exist_ok=True)
                log.info(f"[ illusion_scorer ] sv={sv_path.name} stats={stats_path.name}")
                result = mod.compute_illusion_scores(
                    str(sv_path), str(stats_path), str(out)
                )
                log.info(f"[ illusion_scorer ] 综合幻觉分: {result.get('composite_illusion_score', 'N/A')}")
            else:
                log.warning(f"[ illusion_scorer ] 缺少数据文件，跳过")
                ill_ok = False

        if args.step in ("illusion", "verification") and not args.skip_verification:
            spec = importlib.util.spec_from_file_location(
                "street_view_verifier",
                SCRIPT_DIR / "street_view_verifier.py"
            )
            mod = importlib.util.module_from_spec(spec)
            spec.loader.exec_module(mod)
            ...
            fac_candidates = [
                SCRIPT_DIR / "network_output" / "facility_locations.json",
            ]

            sv_path = next((p for p in sv_candidates if p.exists()), None)
            ns_path = next((p for p in ns_candidates if p.exists()), None)
            fac_path = next((p for p in fac_candidates if p.exists()), None)

            if ns_path and sv_path and fac_path:
                out = SCRIPT_DIR / "verifier_output"
                out.mkdir(exist_ok=True)
                log.info(f"[ street_view_verifier ] manifest={ns_path.name} csv={sv_path.name}")
                result = mod.verify_paths(
                    str(ns_path), str(sv_path), str(fac_path), str(out), 50.0, 100.0
                )
                log.info(f"[ street_view_verifier ] 验证路径数: {result.get('total_paths', 'N/A')}")
            else:
                log.warning(f"[ street_view_verifier ] 缺少数据文件，跳过")
                ver_ok = False

        if args.step == "illusion" and (ill_ok or ver_ok):
            log.info(f"\n{'=' * 50}")
            log.info("幻觉验证完成！启动对照面板：")
            log.info(f"  cd city_twin_output; python -m http.server 8899")
            log.info(f"  访问: illusion_verification_panel.html")
            log.info(f"{'=' * 50}")
