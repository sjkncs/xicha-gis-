...
Write-Host "[1/7] Creating WinRM session to $server..." -ForegroundColor Cyan
try {
    $session = New-PSSession -ComputerName $server -Port $port -UseSSL -Credential $creds -SessionOption $so
    Write-Host "[OK] Session established" -ForegroundColor Green
} catch {
    Write-Host "[FAIL] Could not connect: $_" -ForegroundColor Red
    exit 1
}

Write-Host "[2/7] Copying deployment package..." -ForegroundColor Cyan
try {
    Copy-Item $localZip -Destination "C:\Users\Administrator\Desktop\dp.zip" -ToSession $session -Force
    Write-Host "[OK] Package copied" -ForegroundColor Green
} catch {
    Write-Host "[FAIL] Copy failed: $_" -ForegroundColor Red
    Remove-PSSession $session; exit 1
}

Write-Host "[3/7] Stopping nginx..." -ForegroundColor Cyan
...
Write-Host "[4/7] Cleaning and extracting files..." -ForegroundColor Cyan
Invoke-Command -Session $session -ScriptBlock {
    param($rp)
    $ErrorActionPreference = "Stop"
    ...
    Write-Host "Files extracted to $rp"
    Get-ChildItem $rp | Select-Object Name, Mode | Format-Table
} -ArgumentList $remotePath

Write-Host "[5/7] Writing nginx.conf..." -ForegroundColor Cyan
Invoke-Command -Session $session -ScriptBlock {
    param($rp)
    $ErrorActionPreference = "Stop"
    $conf = @"
...
Write-Host "[6/7] Starting nginx..." -ForegroundColor Cyan
Invoke-Command -Session $session -ScriptBlock {
    $ErrorActionPreference = "SilentlyContinue"
    ...
    else { Write-Host "WARNING: nginx process not found after start" }
}
