# -*- coding: utf-8 -*-
"""
NVIDIA NIM VLM 语义分割 - 无示例值版（避免模板输出）
Llama 3.2 11B Vision, ~7s/张
"""
...
OUT_DIR = BASE_DIR / "segmentation_results_v2"
...
PROMPT = (
    ...
    "- openness: visual openness feeling 1-10 (1=very enclosed, 10=very open)\n"
    "- street_canyon: street canyon effect 1-10 (1=no canyon, 10=high-rise canyon)\n"
    "- building_density: perceived building density 1-10\n"
    "- walkability: walkability perception 1-10\n"
    "- urban_form: village type (城中村/commodity_housing/old_community/new_community/"
    "industrial/commercial/public_space/other)\n"
    "- description_zh: brief description in Chinese\n\n"
...
def parse_json_response(content):
    ...
    try:
        ...
        try:
            ...
            vals = {}
            for k in ["building_pct", "road_pct", "green_pct", "sky_pct",
                       "openness", "street_canyon", "building_density", "walkability"]:
                m = re.search(rf'"{k}"\s*:\s*(\d+(?:\.\d+)?)', raw)
                if m:
...
def analyze_one(item, retry_count=0):
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            elif parsed and "_partial" in parsed:
                return {
                    "status": "partial",
                    "data": parsed,
                    "tokens": usage.get("completion_tokens", 0),
...
def main():
    print("=" * 60)
    print("NVIDIA NIM VLM 语义分割 (纯净版 - 无示例值)")
    print(f"模型: {MODEL_ID}")
    print("=" * 60)

    tasks = load_images()
    print(f"南山区图像: {len(tasks)} 张")

    checkpoint = load_checkpoint()
    remaining = [t for t in tasks if t["path"] not in checkpoint]
    print(f"已处理: {len(checkpoint)} | 剩余: {len(remaining)}")
    ...
    print(f"\n开始处理 {len(remaining)} 张...")
    done_list = list(checkpoint.values())
    results = list(checkpoint.values())
    ok_count = len([x for x in checkpoint.values() if x.get("status") in ("success", "partial")])

    batch_num = len(checkpoint)
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as ex:
        futures = {ex.submit(analyze_one, t): t for t in remaining}
        ...
        for future in as_completed(futures):
            ...
            if res["status"] in ("success", "partial"):
                ...
                print(f"  [{batch_num}/{len(tasks)}] OK  {task['heading']:4s} | "
...
def summarize(results):
    ...
    fields = [("building_pct", "建筑%"), ("road_pct", "道路%"),
              ("green_pct", "绿地%"), ("sky_pct", "天空%")]
    print(f"\n覆盖率统计:")
    for k, name in fields:
        vals = [r.get(k, 0) for r in ok if r.get(k) is not None and isinstance(r.get(k), (int, float))]
        if vals:
            print(f"  {name:6s}: 平均{avg(vals):5.1f}% | 范围 {rng(vals)} | "
                  f"中位数{statistics.median(vals):.1f}%")

    print(f"\n感知评分:")
    for k, name in [("openness", "开阔度"), ("street_canyon", "峡谷"),
                     ("building_density", "建筑密度"), ("walkability", "步行性")]:
        vals = [r.get(k, 0) for r in ok if r.get(k) is not None and isinstance(r.get(k), (int, float))]
        if vals:
