...
# -*- coding: utf-8 -*-
"""
SegFormer B3 ADE20K - v5
直接用torch.load加载权重，绕过HuggingFace的from_pretrained验证问题
"""
...
MODEL_PATH = Path("/root/autodl-tmp/models/hub/models--nvidia--segformer-b3-finetuned-ade-512-512/snapshots/default")
DATA_DIR = Path("/root/autodl-tmp/streetview_analysis/images")
OUT_DIR = Path("/root/autodl-tmp/outputs/segmentation")
VIZ_DIR = OUT_DIR / "viz"
CHECKPOINT = OUT_DIR / "checkpoint.json"
LOG_FILE = Path("/root/gis_project/logs/seg_v5.log")
...
def load_model():
    """Load SegFormer B3 using torch.load to bypass HF validation."""
    from transformers import (
        SegformerConfig, SegformerImageProcessor, SegformerForSemanticSegmentation
    )
    ...
    config = SegformerConfig.from_pretrained(str(MODEL_PATH))
    model = SegformerForSemanticSegmentation(config)

    bin_path = MODEL_PATH / "pytorch_model.bin"
    log("Loading weights from: {}".format(bin_path))
    state_dict = torch.load(bin_path, map_location="cpu")
    missing, unexpected = model.load_state_dict(state_dict, strict=False)
    if missing:
        log("Missing keys: {}".format(missing[:5]))
    if unexpected:
        log("Unexpected keys: {}".format(unexpected[:5]))
    del state_dict
    log("Weights loaded!")
...
def main():
    log("=" * 60)
    log("SegFormer B3 ADE20K - v5 (torch.load)")
    ...
    all_jpg = sorted(list(DATA_DIR.rglob("*.jpg")) + list(DATA_DIR.rglob("*.JPG")))
    ...
    for i, img_path in enumerate(remaining):
        ...
        if (i + 1) % 10 == 0:
            ...
            log("  Progress: {}/{} | Elapsed:{:.1f}min | Mem:{:.2f}GB".format(
                i+1, len(remaining), elapsed_total/60, mem))
