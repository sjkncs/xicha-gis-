...
# Show actual category names for debugging
all_cats = defaultdict(int)
for r in nanshan:
    for k, v in r["categories"].items():
        all_cats[k] += v
print("实际类别名:", list(all_cats.keys()))
...
共采集街景图像 294 张，其中南山区 136 张。
图像采集自腾讯/百度街景API，拍摄时间为2022年。
采样密度约为每500米一个采样点，每点采集东、西、南、北四个方向。

\subsection{目标检测模型}
本研究采用YOLO11x目标检测模型，基于COCO-80类别数据集进行预训练。
...
\begin{tabular}{ccccc}
\hline
等级 & 评分范围 & 含义 & 颜色标记 \\ \hline
畅通 & $[0, 10)$ & 无明显障碍 & \cellcolor{green} \\
较畅通 & $[10, 20)$ & 少量非机动车 & \cellcolor{lightgreen} \\
一般 & $[20, 40)$ & 存在占道 & \cellcolor{yellow} \\
较困难 & $[40, 60)$ & 严重占道 & \cellcolor{orange} \\
困难 & $[60, 80)$ & 严重影响通行 & \cellcolor{red} \\
严重 & $[80, 100]$ & 几乎无法通行 & \cellcolor{darkred} \\ \hline
\end{tabular}
\end{table}
""")
...
ns_scores_arr = np.array(ns_scores)
...
# Category stats
all_cats2 = defaultdict(int)
for r in nanshan:
    for k, v in r["categories"].items():
        all_cats2[k] += v
total_det = sum(all_cats2.values())
...
# Find top category
top_cat = max(all_cats2.items(), key=lambda x: x[1])
...
for label, cnt, pct in rating_dist:
    bar = "█" * int(pct / 2)
    print(f"  {label}级: {cnt}张 ({pct:.1f}\%) {bar}")
...
for cat, cnt in sorted(all_cats2.items(), key=lambda x: -x[1]):
    pct = cnt / total_det * 100
    bar = "█" * int(pct / 2)
    print(f"  {cat}: {cnt}次 ({pct:.1f}\%) {bar}")

print(f"""
其中 {top_cat[0]} 占道比例最高，达 {top_pct:.1f}\%，
...
\hline
街道 & 样本数 & 均值 & 中位数 & 最高分 & 标准差 & 评级 \\ \hline""")
...
\item \textbf{{非机动车占道次之}}：摩托车/电动车占道 {all_cats2.get("摩托车/电动车", 0)} 次，
  说明非机动车停车设施配套不完善。
...
print("\n" + "=" * 70)
print(f"街道数量: {len(street_data)}")
print(f"障碍评分均值: {ns_mean:.2f}")
print(f"最高障碍街道: {worst['name']} ({worst['mean']:.1f})")
print(f"最低障碍街道: {best['name']} ({best['mean']:.1f})")
print(f"主要障碍类型: {top_cat[0]} ({top_pct:.1f}%)")
print("=" * 70)
