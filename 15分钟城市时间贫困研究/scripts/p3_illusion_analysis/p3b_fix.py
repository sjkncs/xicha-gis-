...
# DEBUG: 检查apply前results列
_needed = ['SAI','SAI_night','TPI','gap']
_present = [c for c in _needed if c in results.columns]
print("DEBUG before P3 section - results cols: {}".format(results.columns.tolist()[:10]))
print("DEBUG needed {} present: {}".format(_needed, _present))
# 强制检查
assert 'SAI' in results.columns, "SAI not in results!"
assert 'SAI_pct' not in results.columns, "SAI_pct already exists!"
...
def classify_quadrant(row):
    ...
    if pct >= median_pct and tpi >= 0:
        # Q1 (右上): 高空间可达 + 正TPI = 时间幻觉
        # → 周边设施充足, 但夜间变差 → 可达性幻觉
        return 'Q1-Temporal Illusion'
    elif pct >= median_pct and tpi < 0:
        # Q2 (左上): 高空间可达 + 负TPI = 夜间优势
        return 'Q2-Night Advantage'
    elif pct < median_pct and tpi >= 0:
        # Q3 (右下): 低空间可达 + 正TPI = 双重剥夺
        return 'Q3-Dual Deprived'
    else:
        # Q4 (左下): 低空间可达 + 负TPI = 空间孤立(但夜间相对均衡)
        return 'Q4-Spatial Isolated'

print("DEBUG classify_quadrant - SAI_pct in results: {}".format('SAI_pct' in results.columns))
print("DEBUG results columns at classify point: {}".format(results.columns.tolist()))
results['quadrant'] = results.apply(classify_quadrant)
...
print("\n[Correlations]:")
for a, b, lbl in [('SAI','TPI','','TPI'), ('SAI','SAII',''), ('TPI','SAII','')]:
